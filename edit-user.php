                        ...
                        <div class="col-lg-12">
                            ...
                            <form method="post" action="">
                                ...
                                if (isset($_GET['edit_user'])) {
                                    $getId = $_GET['edit_user'];

                                    $getquery = $pdo->prepare("SELECT * FROM user WHERE user_id= '{$getId}'");
                                    $getquery->execute();
                                ...
                                if (isset($_POST['edit_user'])) {
                                    ...
                                    $email = $_POST['email'];
                                    $userflow = $_POST['userflow'];

                                    $sql = $pdo->prepare("UPDATE user SET fullname=:fullname, email=:email, userflow=:userflow");
                                    $sql->bindValue(':fullname', $fullname);
                                    $sql->bindValue(':email', $email);
                                    $ql->bindValue(':userflow', $userflow);
                                    $sql->execute();
                                }
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <input aria-describedby="" class="form-control" id="" value="<?php echo $fullname; ?>" name="fullname" placeholder="Full Name" type="text">
                                            </div>

                                            <div class="col-md-6">
                                                <label for="">Email</label>
                                                <input aria-describedby="" class="form-control" id="" value="<?php echo $email; ?>" name="email" placeholder="Email" type="email">
                                            </div>
                                        </div><br>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <label for="">Role</label>
                                                <select name="userflow" class="form-control" id="">

                                                    <?php

                                                    $edit_query = $pdo->prepare("SELECT * FROM role");
                                                    $edit_query->execute();

                                                    while ($row = $edit_query->fetch()) {
                                                        $rid = $row['id'];
                                                        $role = $row['role'];
                                                    ?>
                                                        <option value="<?php echo $role; ?>" <?php if ($userflow == $role) {
                                                                                                    echo "selected";
                                                                                                } ?>><?php echo $userflow; ?></option>
